...
create table
    mkuliah (
        kode_kul char(7) not null UNIQUE PRIMARY key,
        nama_kul char(20) not null,
        sks decimal(2) not null,
        smt_tawar char(1) check (smt_tawar between '1' and '8'),
        klp_kul char(1) check (klp_kul between '1' and '3') DEFAULT '1',
        progdi char(1) check (progdi between '1' and '5')
    );
...
--* 10. 0. Gantilah nama mata kuliah ‘Kewiraan’ menjadi ‘Pendidikan Kewarganegaraan’
update mkuliah
set
...
-- select * from mkuliah group by klp_kul;
SELECT
    klp_kul,
    COUNT(kode_kul)
FROM
