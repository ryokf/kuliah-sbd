...
create table mhs (
    nim char(14) not null unique primary key, nama char(25) not null unique, kelamin char(1) check (kelamin in ('L', 'P')), agama char(1) check (
        agama in ('1', '2', '3', '4', '5')
    ), tgl_lhr date
);
...
alter table mhs change column kelamin sex CHAR(1);
